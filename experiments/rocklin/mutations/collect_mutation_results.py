...
results_files = glob('*.tsv')

print(results_files)

for suffix in ['_full', '_hbonds']:
    for file in sorted(glob('*' + suffix + '.tsv')):
        df = read_csv(file, sep='\t')
        pearson = corrcoef(df['stabilityscore'], df['neglogp'])[0,1]
        spearman = spearmanr(df['stabilityscore'], df['neglogp'])[0]
...
# [
#     'HHH_rd2_0134.pdb_full.tsv', 'HEEH_rd3_0872.pdb_full.tsv',
#     'EEHEE_rd3_0037.pdb_full.tsv', 'HEEH_rd3_0726.pdb_full.tsv',
#     'HHH_rd3_0138.pdb_full.tsv', 'HEEH_rd2_0779.pdb_full.tsv',
#     'EEHEE_rd3_1702.pdb_full.tsv', 'EEHEE_rd3_1716.pdb_full.tsv',
#     'HEEH_rd3_0223.pdb_full.tsv', 'EEHEE_rd3_1498.pdb_full.tsv'
# ]
