...
from gpn.struct2seq.self_attention import gather_edges, gather_nodes, Normalize
...
class PositionalEncodings(nn.Module):
    ...
    def forward(self, E_idx):
        ...
        frequency = torch.exp(
            torch.arange(0, self.num_embeddings, 2, dtype=torch.float32) * -(np.log(10000.0) / self.num_embeddings)
        )
        # Grid-aligned
...
class ProteinFeatures(nn.Module):
    ...
    def forward(self, X, L, mask):
        ...
        # DEBUG
        # U = (np.nan * torch.zeros(X.size(0), X.size(1), X.size(1), 3)).scatter(
        #     2, E_idx.unsqueeze(-1).expand(-1, -1, -1, 3), E[:, :, :, :3]
        # )
